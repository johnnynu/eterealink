...
- Map `aurealink.app` and `www.aurealink.app` directly to the frontend service with Google-managed TLS. Authorize the new, legacy, and deterministic frontend hostnames in Firebase Authentication during the migration, and add their exact origins to the Cloud Storage bucket CORS policy.
